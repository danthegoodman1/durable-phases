...
import type { BenchmarkCounters } from "./workload.js"
import { createBenchmarkCounters } from "./workload.js"
import type {
  NullBenchmarkMode,
  NullBenchmarkOptions,
  NullBenchmarkResult,
} from "./null.js"

export type NullMultiProcessBenchmarkOptions = {
...
export type NullProcessResult = {
  processIndex: number
  workflows: number
  workflowOffset: number
  result: NullBenchmarkResult
}

export type NullMultiProcessBenchmarkResult = {
  backend: "null-multiprocess"
  mode: NullBenchmarkMode
  options: NullMultiProcessBenchmarkOptions
  elapsedMs: number
  processingMs: number
  processes: number
  ...
  processResults: NullProcessResult[]
}

export type NullProcessRunner = (
  processIndex: number,
  options: NullBenchmarkOptions,
) => Promise<NullBenchmarkResult>

const defaultOptions: NullMultiProcessBenchmarkOptions = {
  mode: "mixed",
  workflows: 1_000,
  processes: 4,
  workersPerProcess: 4,
...
async function main(): Promise<void> {
  const options = parseNullMultiProcessBenchmarkArgs(process.argv.slice(2))
  if (!options.json) {
    process.stdout.write(
      `Running multi-process null benchmark with ${options.workflows} workflows, ${options.processes} processes, ${options.mode} mode, ${options.workersPerProcess} workers/process, ${options.shardsPerProcess} shards/process...\n\n`,
    )
  }
  const result = await runNullMultiProcessBenchmark(options)
  if (options.json) {
...
export async function runNullMultiProcessBenchmark(
  options: NullMultiProcessBenchmarkOptions,
  runner: NullProcessRunner = runNullBenchmarkProcess,
): Promise<NullMultiProcessBenchmarkResult> {
  ...
  const processResults = await Promise.all(
    processInputs.map(({ workflows, workflowOffset }, processIndex) =>
      runner(processIndex, {
        ...
        shards: options.shardsPerProcess,
...
export function parseNullMultiProcessBenchmarkArgs(
  args: string[],
...
async function runNullBenchmarkProcess(
  processIndex: number,
  options: NullBenchmarkOptions,
): Promise<NullBenchmarkResult> {
  const tsxCli = fileURLToPath(new URL("../../node_modules/tsx/dist/cli.mjs", import.meta.url))
  const benchmarkScript = fileURLToPath(new URL("./null.ts", import.meta.url))
  const args = [
    tsxCli,
    benchmarkScript,
    "--json",
  ...
  const output = await spawnCollect(process.execPath, args, {
    DURABLE_NULL_PROCESS_INDEX: String(processIndex),
  })
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) {
        ...
        reject(new Error(`Null benchmark subprocess exited with ${code}\n${stderr}\n${stdout}`))
      }
    })
  })
}

function parseJsonOutput(output: string): NullBenchmarkResult {
  const start = output.indexOf("{")
  const end = output.lastIndexOf("}")
  if (start < 0 || end <= start) {
    throw new Error(`Subprocess did not print JSON: ${output}`)
  }
  return JSON.parse(output.slice(start, end + 1)) as NullBenchmarkResult
}
...
function printHelp(): void {
  process.stdout.write(`Multi-process null durability benchmark

Launches multiple subprocesses running the benchmark-only null provider and
...
function printResult(result: NullMultiProcessBenchmarkResult): void {
  process.stdout.write(`Multi-process null durability benchmark
  mode: ${result.mode}
