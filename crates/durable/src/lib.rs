...
pub use postgres::{PostgresDurabilityProvider, PostgresDurabilityProviderOptions};
pub use sqlite::{
    SqliteDurabilityOptions, SqliteDurabilityProvider, SqliteShardFileDurabilityProvider,
};
...
impl ShardRouter {
    ...
    async fn reset_from_store(&self, store: Store) -> Result<(), WorkflowError> {
        let directory = store
            .instances
            .values()
            .map(|instance| {
            ...
            .collect();
        let instance_cache = store.instances.clone();
        let initial_shards = split_store_by_shard(store);
        ...
        *self
            ...
            .initial_shards
            .lock()
            .map_err(|_| WorkflowError::new("shard initial state lock poisoned"))? = initial_shards;
        *self
            .inner
            .directory
            .lock()
            .map_err(|_| WorkflowError::new("instance directory lock poisoned"))? = directory;
        *self
            .inner
            .instance_cache
            .lock()
            .map_err(|_| WorkflowError::new("instance cache lock poisoned"))? = instance_cache;
        Ok(())
    }
...
async fn create_child_instance_would_mutate(
    provider: &ShardRouter,
    input: &CreateChildInstanceInput,
) -> Result<bool, WorkflowError> {
    let store = provider.snapshot_store().await?;
    if store.children.iter().any(|record| {
        record.parent_workflow_id == input.parent_workflow_id
            && record.parent_run_id == input.parent_run_id
            && record.activation_id == input.activation_id
            && record.key == input.key
    ...
    if let Some(policy) = input.workflow_id_reuse_policy {
        if let Some(existing) = latest_workflow_run(&store, &input.workflow_id) {
            if !should_create_workflow_run(policy, &existing.status) {
                return Ok(true);
            }
        }
    ...
    Ok(input.conflict_policy == ConflictPolicy::TerminateExisting
        || !store
            .instances
            .contains_key(&instance_key(&input.workflow_id, &input.run_id)))
}

async fn cancel_child_would_mutate(
    provider: &ShardRouter,
    input: &CancelChildInput,
) -> Result<bool, WorkflowError> {
    Ok(provider
        .snapshot_store()
        .await?
...
async fn activation_release_would_mutate(
    provider: &ShardRouter,
    activation_id: &str,
    worker_id: &str,
) -> Result<bool, WorkflowError> {
    Ok(provider.snapshot_store().await?.tasks.values().any(|task| {
        task.activation_id == activation_id && task.claim_owner_id.as_deref() == Some(worker_id)
    }))
}

async fn shard_release_would_mutate(
    provider: &ShardRouter,
    shard_id: u32,
    owner_id: &str,
) -> Result<bool, WorkflowError> {
